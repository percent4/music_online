...
public class SongController {
    ...
    public String submitForm(@ModelAttribute Song_Info songInfo, Map <String, Object> map) throws Exception {
        ...
        writeTXT(song_name,platform); // 写入到history.csv文件中，用来显示搜索历史
        ...
        ArrayList<String> records = read_txt();
        ArrayList<String> columns = new ArrayList<>();
        for(int i=0;i<records.size();i++){
            columns.add(records.get(i));
        }
    ...
    public static void writeTXT(String song_name, String platform) {
        ...
        try {
            ...
            byte[] write_content = new String(song_name+','+Platform+','+bjSdf.format(now)+'\n').getBytes("UTF-8");
            writer.write(new String(write_content,"UTF-8"));
            System.out.println("Write to history.txt successfully!");
        }
        catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static ArrayList<String> read_txt() {
        ...
        try {
            ...
            while ((tempString = reader.readLine()) != null)
                content.add(tempString); // 在列表中添加国家名称
